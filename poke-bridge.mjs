...
server.tool(
  ...
  async ({ message }) => {
    ...
    return {
      content: [{ type: "text", text: result.ok ? `Sent to Poke: "${message}"` : `Error: ${result.text}` }],
      isError: !result.ok,
    };
  }
);
...
server.tool(
  ...
  async ({ recipient, subject, context }) => {
    ...
    return {
      content: [{ type: "text", text: result.ok ? `Asked Poke to draft email to ${recipient} re: ${subject}` : `Error: ${result.text}` }],
      isError: !result.ok,
    };
  }
);
